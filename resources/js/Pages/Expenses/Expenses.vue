...
const filteredexpenses = computed(() => {
    ...
    if (!searchQuery.value) {
        return props.expensesArr;
    }

    const query = searchQuery.value.toLowerCase();

    return props.expensesArr.filter((expense) => {
        // Cari berdasarkan judul pekerjaan (title) atau kriteria lainnya
        return (
            expense.name.toLowerCase().includes(query) ||
            expense.amount.includes(query)
        );
    });
});
...
const Delete = async (id) => {
    ...
    try {
        const response = await axios.post("/api/delete-expenses", payload);
...
<template>
    <Head title="Expenses (Pengeluaran)" />
    <Main>
        ...
        <div class="px-4 sm:px-6 lg:px-8">
            <div class="sm:flex sm:items-center">
                <div class="sm:flex-auto">
                    <h1 class="text-base font-semibold text-white">Expenses</h1>
                    <p class="mt-2 text-sm text-gray-300">
                        Pengeluran selama ini.
                    </p>
                </div>
                <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                    <a
                        :href="route('admin.expenses.add')"
